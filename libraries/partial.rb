...
class Chef
  ...
  class Provider
    class Partial < Chef::Provider
      ...
      def action_run
        partial = new_resource.name
        Chef::Log.info "Executing partial run list: #{new_resource.run_list}"
        runner = Chef::PartialRun.new(
          partial,
          new_resource.attributes,
          new_resource.arguments
        )
        runner.partial_run(new_resource.run_list)
        unless runner.run_status.updated_resources.empty?
          new_resource.updated_by_last_action(true)
        end

        if new_resource.save
          node.consume_attributes(runner.cleanse_attrs)

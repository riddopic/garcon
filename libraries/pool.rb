...
class Thread::Pool
  ...
  def initialize(min, max = nil, &block)
		...
		@mutex.synchronize { min.times { spawn_thread } }
  end
  ...
  def shutdown?
	...
	# Enable idle trimming. Unneeded threads will be deleted after the given
  # number of seconds of inactivity. The minimum number of threads is respeced.
	...
	# Process Block when there is a idle worker if no block is passed it returns
	def idle(*args, &block)
		while !idle?
			@done_mutex.synchronize {
				break if idle?
				...
				task = @mutex.synchronize {
					if @todo.empty?
						while @todo.empty?
							...
							if @idle_trim && @spawned > @min
								check_time = Time.now + @idle_trim
								@cond.wait @mutex, @idle_trim
								@trim_requests += 1 if Time.now >= check_time &&
                  @spawned - @trim_requests > @min
							else
								@cond.wait @mutex
							end
